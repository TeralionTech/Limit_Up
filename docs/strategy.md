...
- **停止條件(兩者任一先發生)**:
  ...
  - **(B) 第一盤成交資訊來**(`first_trade_fired`)且該筆為**非致命**拒單 → 收手停送 + 撤預掛 P
    (此時無委託成功 → 用 `cancel_symbol_orders` 撤,會釋放 P 保留預算)。
- **致命拒單(價格穩定 / 全額 / 預收 / 圈存)→ 停市價、但保留預掛 P 續守、不動預算**。
  即使第一盤成交已來也保留 P(安全防線優先):價格穩定(6144)時市價一定被拒,但漲停價限價單 P 仍是有效
  排隊單,是唯一活路,撤掉就保證槓龜。(全額交割類 P 通常在預掛時就被拒/被 T30 擋,9:00 已無 P 可留。)
- 其他中止:kill switch 關 / 該檔淘汰或出場 / 09:05 時間兜底 / 13:23 / 總曝險硬上限 / 處置股(禁市價,只留預掛)。
